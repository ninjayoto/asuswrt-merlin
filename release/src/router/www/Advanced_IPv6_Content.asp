...
var ipv6_proto_last = '<% nvram_get("ipv6_service_last"); %>';
...
function showInputfield(v){
	...
	if(v == "static6"){
		...
		if((v != ipv6_proto_orig) && (v != ipv6_proto_last)){
...
function showInputfield2(s, v){
	...
	if(s=='ipv6_6rd_dhcp'){
		...
		if(v != ipv6_tun6rd_dhcp || (document.form.ipv6_service.value != ipv6_proto_orig && document.form.ipv6_service.value != ipv6_proto_last)){
//			var calc_hex = calcIP6(wan0_ipaddr);
	...
	if(validForm()){
		...
		if(document.form.ipv6_autoconf_type[1].checked && document.form.ipv6_service.value=="dhcp6"){
			if(document.form.ipv6_dhcp_pd[1].checked){
				...
				document.form.ipv6_dhcp_start.disabled = false;
				document.form.ipv6_dhcp_start.value = "::"+document.form.ipv6_dhcp_start_start.value;
				document.form.ipv6_dhcp_end.disabled = false;
				document.form.ipv6_dhcp_end.value = "::"+document.form.ipv6_dhcp_end_end.value;
			}
		}else{
		...
		// Save last active ipv6 service variable if switching to disabled
...
function showInfo(){
		if((document.form.ipv6_service.value == ipv6_proto_orig) || (document.form.ipv6_service.value == ipv6_proto_last)){
